...
#/ Usage: deploy.sh remote_user private_key artifact_url
#/ Description: Deploy Vert.x Starter artifact on start.vertx.io
#/ Examples:
#/ Options:
...
if [[ "${BASH_SOURCE[0]}" = "$0" ]]; then
    ...
    info "Deploying with user ${remote_user}"
    
    ansible-playbook -v --private-key=${private_key} -u ${remote_user} -i hosts deploy.yml --ask-vault-pass
fi